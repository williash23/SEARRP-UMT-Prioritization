...
#     2. Total area for overall prioritization 410,000 ha + 20,000 ha = 430,000 ha
#        - 6 individual inputs prioritized separately for whole of Sabah then prioritized together 
#          for 430,000 ha across all Sabah
...
# =============================================================================
	...
	load(file = "C:/Users/saraw/Desktop/planning_unit_grids/ecolinc_grid.Rdata")
	...
	area_thresh <- 100000000
	area_thresh2 <- 100000000
	ecolinc <- st_as_sf(raster::aggregate(as(pu_crock_kina, "Spatial"))) %>%
		st_buffer(-2000) %>%
		drop_crumbs(area_thresh) %>%
		fill_holes(area_thresh2) %>%
		st_buffer(-2000)
	
	locked_in_id <- as.numeric(st_intersects(pu_sf, ecolinc, sparse = FALSE))
	locked_in_tmp <- pu_sf[which(locked_in_id == 1),]
	locked_in <- as(locked_in_tmp, "Spatial")
...
# =============================================================================
	...
	p_vert2 <- problem(x = pu_in, features = vert_feat_in_single, cost_column = "area_h") %>%
		add_max_features_objective(440000) %>%
...
# =============================================================================	
	...
	p3 <- problem(x = pu_in, features = all_feat_in, cost_column = "area_h") %>%
		add_max_utility_objective(440000) %>%
